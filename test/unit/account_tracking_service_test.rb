  ...
  class AccountTrackingServiceTest < ActiveSupport::TestCase
    ...
    test 'get_account_statuses_by_system returns correct statuses' do
      ...
      result.each do |status|
        assert status.key?(:user_id)
        assert status.key?(:user_type)
      ...
      assert_equal 'emp001', result[0][:user_id]
      assert_equal 'active', result[0][:status]
    ...
    test 'get_account_statuses_by_system filters by system' do
      ...
      assert_equal 1, result.length
      assert_equal 'emp002', result[0][:user_id]
      assert_equal 'AIX', result[0][:account_type]
    ...
    test 'get_account_statuses_by_system filters by as_of_time' do
      ...
      user_ids = result.map { |r| r[:user_id] }.sort
      assert_equal ['emp001', 'emp002'], user_ids
    ...
    test 'get_account_statuses_by_system defaults to current time when as_of_time not provided' do
      ...
      assert_equal 2, result.length
      assert_includes result.map { |r| r[:user_id] }, 'emp001'
      assert_includes result.map { |r| r[:user_id] }, 'emp002'
    end
